...
- `audio.wav` or `audio.flac` — reference audio (about 3–15 seconds of clear single-speaker speech)
- `ref_text.txt` — transcript of what is spoken in the audio (must match closely for best clone quality)

A folder is a valid voice when it has **one** of those audio files plus `ref_text.txt`. If both `audio.wav` and `audio.flac` are present, Buddy prefers `audio.wav`.
...
2. Add `audio.wav` or `audio.flac`, and `ref_text.txt`.
...
Default backend is Qwen3 (`$ttsBackend = "qwen3"` in `start-speech-to-speech.ps1`). Optional Pocket TTS uses preset catalog voices only; cloning from your own reference audio requires accepting Kyutai's terms.
